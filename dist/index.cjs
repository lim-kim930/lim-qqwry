...
class LimQqwry {
    ...
    getIPData(ip, withNext) {
        ...
        return withNext ? { data, next } : Object.assign(Object.assign({}, loc), { ip: LimQqwry.intToIP(data.start_ip) });
    }
    searchIP(ip) {
    ...
    toJson() {
        ...
        let result = [];
        while (ip < 4294967295) {
            const { data, next } = this.getIPData(ip, true);
            if (!next) {
    ...
    static getStartIpInt(ip, result) {
    ...
    GetMiddleOffset(begin, end, recordLength) {
